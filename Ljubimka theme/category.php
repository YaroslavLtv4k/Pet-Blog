...
<div class="animal-header">
	<div class="container">
		<div class="content">
			<h2><?php echo get_field('parent_category', get_queried_object()); ?></h2>
			<div class="sub-plus-button">
				<div class="subcategories">
					<?php 
						$current_category = get_category( get_query_var( 'cat' ), false ); 
						$child = get_category($current_category->term_id);
						// Get Parent Id
						$parent = $child->parent;
						$parent_id = get_category($parent);
						$parent_id = $parent_id->term_id;
						$parent_name = mb_convert_case(get_cat_name( $parent_id ), MB_CASE_LOWER, "UTF-8");

						// Display menu for current parent page
								
						if (is_subcategory()) {
							// if current category - subcategory
						 	$categoriesMenuFor = 'Subcategories for ' . $parent_name;
						 }elseif(!is_subcategory()){
						 	// if current category - parent category
						 	$categoriesMenuFor = 'Subcategories for ' . mb_convert_case($current_category->cat_name, MB_CASE_LOWER, "UTF-8");

						}

						wp_nav_menu(
							array(
								'theme_location' => 'subcategories-menu',
								'menu' => $categoriesMenuFor,
							)
						) 
				...
				<div class="random-button">
					<a id="random-post" href="http://ljubimka.com/

					<?php 
					echo (is_subcategory())?$parent_name:mb_convert_case($current_category->cat_name, MB_CASE_LOWER, "UTF-8");  
					echo '/';
					echo (is_subcategory())?$current_category->slug . '/':'';

					?>

					?redirect_to=random">Random Post</a>
				</div>
			</div>
		</div>
